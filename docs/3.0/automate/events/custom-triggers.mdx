...
results to a remote filesystem and you receive a Slack message, combine them with a 'compound' trigger:
  ...
  "triggers": [
    {
      "type": "event",
      "posture": "Reactive",
      "expect": ["prefect.slack.message.received"],
      "match_related": {
        "prefect.slack.channel": "deployments",
        "prefect.slack.keyword": "Start Deployment"
      }
    },
    {
      "type": "event",
...
This trigger fires once it sees the specified Slack message in the `deployments` channel and at least one of each of the underlying event triggers fire within the time frame specified. Then the trigger resets its state and fires the next time these events all happen.
The order the events occur doesn't matter, just that all of the events occur within one hour.

If you want a flow run to complete prior to starting to watch for those three events and the Slack message, you can combine the entire
...
In this case, the trigger only fires if it sees the `daily-export-initiator` flow complete, the three
files written by the other flows, and the specified Slack message.
...
flows don't write their result files until three hours later, or the Slack message isn't received within the time frame,
this trigger won't fire. Placing these time constraints
