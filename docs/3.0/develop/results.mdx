...
various keyword arguments and the `PREFECT_RESULTS_PERSIST_BY_DEFAULT` setting. Additionally, you can control task-specific persistence using the `PREFECT_TASKS_DEFAULT_PERSIST_RESULT` setting, which allows you to enable or disable result persistence for tasks independently of flows.
...
- `PREFECT_TASKS_DEFAULT_PERSIST_RESULT`: If set to `True`, results will be persisted by default for all tasks. Set to `False` to disable persistence by default for tasks. This setting overrides the behavior set by a parent flow or task.
- `PREFECT_RESULTS_PERSIST_FLOWS_BY_DEFAULT`: Controls persistence for flows specifically.

These settings allow for more granular control over result persistence, helping to manage memory usage and storage requirements effectively.
...
You can now also control result persistence behavior separately for tasks and flows using the following environment variables:
- `PREFECT_RESULTS_PERSIST_TASKS_BY_DEFAULT`: Controls whether results are persisted by default for tasks.
- `PREFECT_RESULTS_PERSIST_FLOWS_BY_DEFAULT`: Controls whether results are persisted by default for flows.
