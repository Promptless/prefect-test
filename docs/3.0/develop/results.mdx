...
various keyword arguments and the `PREFECT_RESULTS_PERSIST_BY_DEFAULT` setting. Additionally, you can control task-specific persistence using the `PREFECT_TASKS_DEFAULT_PERSIST_RESULT` setting.
...
Additionally, you can control result persistence separately for tasks and flows using the following settings:
...
These settings allow for more granular control over persistence behavior, enabling you to manage memory usage and storage more effectively.
...
Additionally, you can control the default persistence behavior for tasks separately from flows using the `PREFECT_TASKS_DEFAULT_PERSIST_RESULT` setting. This allows you to enable or disable result persistence by default for all tasks, independent of the flow settings. If set to `False`, it will override the behavior set by a parent flow or task.
### Result storage
...
You can also control the default persistence behavior for tasks and flows separately using the following environment variables:
- `PREFECT_TASKS_DEFAULT_PERSIST_RESULT`: If set to `True`, results will be persisted by default for all tasks. Set to `False` to disable persistence by default for tasks. This setting will override the behavior set by a parent flow or task.
- `PREFECT_RESULTS_PERSIST_BY_DEFAULT`: Controls the default persistence for flows.
