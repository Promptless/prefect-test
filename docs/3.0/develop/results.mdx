...
Once result persistence is enabled - whether through the `PREFECT_RESULTS_PERSIST_BY_DEFAULT` setting, `PREFECT_TASKS_DEFAULT_PERSIST_RESULT` setting, or through any of the mechanisms [described below](/3.0/develop/results#enabling-result-persistence) - Prefect's default result storage configuration is activated.
...
Additionally, you can control task result persistence globally using the `PREFECT_TASKS_DEFAULT_PERSIST_RESULT` setting. If set to `True`, results will be persisted by default for all tasks. Setting it to `False` will disable persistence by default for tasks, overriding the behavior set by a parent flow or task.
### Result storage
...
You can also control result persistence behavior separately for tasks and flows using the following environment variables:
...
- `PREFECT_RESULTS_PERSIST_BY_DEFAULT`: Controls the default result persistence for flows.
