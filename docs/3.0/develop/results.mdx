...
Additionally, you can control result persistence separately for tasks and flows using the following environment variables:
...
- `PREFECT_RESULTS_PERSIST_FLOWS_BY_DEFAULT`: Controls the default persistence behavior for flows.

These settings provide more granular control over result persistence, allowing you to manage memory usage and storage more effectively.
...
You can now also control result persistence for tasks separately from flows using the `PREFECT_TASKS_DEFAULT_PERSIST_RESULT` setting. This allows you to specify whether task results should be persisted by default, independent of the flow's settings.
