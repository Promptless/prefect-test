...
You can control the default behavior for result persistence using the following settings:

- `PREFECT_RESULTS_PERSIST_BY_DEFAULT`: Controls whether results are persisted by default for both flows and tasks.
- `PREFECT_TASKS_DEFAULT_PERSIST_RESULT`: Specifically controls whether results are persisted by default for tasks only. This setting can override the flow's persistence setting for tasks.

These settings can be configured in your environment or via the TOML configuration file.
