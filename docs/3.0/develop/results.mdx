...
You can also control the default persistence behavior for tasks using the `PREFECT_TASKS_DEFAULT_PERSIST_RESULT` setting. 
Set it to `True` to enable result persistence by default for all tasks, or `False` to disable it. 
This setting will override the behavior set by a parent flow or task unless explicitly configured otherwise.
