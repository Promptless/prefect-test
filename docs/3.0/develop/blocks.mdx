...
Similarly, you can use `SecretStr` with other credentials, such as API tokens for services like Slack.

Here's an example of a `SlackCredentials` block that uses `SecretStr`:
...
Since `slack_api_token` has the `SecretStr` type hint assigned to it, 
the value of that field is not exposed if the object is logged:

```python
slack_credentials_block = SlackCredentials(
    slack_api_token="xoxb-1234567890-abcdefghijklmnop"
)
