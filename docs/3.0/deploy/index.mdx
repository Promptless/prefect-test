...
- **`worker_status_filter`**: an optional field that filters workers based on their status, such as active or inactive. This field optimizes deployment by only displaying setup instructions when necessary.