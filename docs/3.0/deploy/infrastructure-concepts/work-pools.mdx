...
**Logging Configuration in Work Pools**

If you encounter issues with log levels being altered, such as info level logs being remapped to error level logs, it may be due to the base job template configuration in your work pool. To address this, review and adjust the logging settings in your work pool's job template to ensure logs are captured at the desired level.
...
| <span class="no-wrap">`--base-job-template`</span> | The path to a JSON file containing the base job template to use. If unspecified, Prefect uses the default base job template for the given worker type. Ensure that the base job template is correctly configured to handle log levels as expected. |
...
```json
{
  ...
  "variables": {
    ...
    "properties": {
      ...
      "log_level": {
        "title": "Log Level",
        "description": "The log level to use for flow run processes. This can be used to control the verbosity of logs.",
        "type": "string",
...
- set the log level for flow run processes
