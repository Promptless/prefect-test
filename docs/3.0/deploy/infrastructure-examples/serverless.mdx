...
```
```bash
prefect work-pool create --type cloud-run:push --provision-infra my-cloud-run-pool --check-active-workers
```

The `--provision-infra` flag allows you to select a GCP project to use for your work pool and automatically 
configures it to execute flows through Cloud Run. The optional `--check-active-workers` flag enables the system to 
detect active workers in the work pool and suppresses unnecessary worker setup instructions if active workers are 
already present.
