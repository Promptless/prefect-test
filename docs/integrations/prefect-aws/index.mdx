...
### Send Slack notifications from flows running on AWS

Send messages to a Slack channel from your flows running on AWS.

First, set up your Slack webhook URL and save it in a `SlackWebhook` block.
...
SlackWebhook(
    url="SLACK-WEBHOOK-URL-PLACEHOLDER"
).save("BLOCK-NAME-PLACEHOLDER")
```

Here's how to use the saved Slack webhook in your flow:
...
def slack_notification_flow():
    ...
    slack_webhook.notify("Hello, Prefect and Slack!")

if __name__ == "__main__":
    slack_notification_flow()
```

You can run this flow on AWS ECS or any other AWS compute service as shown in previous examples.
